...
//
// This program checks that there exists a *left identity", i.e., an element I
// such that I*x = x for all x, but fails to check that I is also a *right identity*,
// i.e., that x*I = x for all x.  (Note that a monoid requires a 2-sided identity).


import java.io.*;

public class GroupthinkWA1{
	...
	private static int findIdentity(){
		for (int i = 0; i < n; i++){
			int count = 0;
			for (int j = 0; j < n; j++){
				if (table[i][j] == j){     // the error is here
					count++;
				} // if
			} // for j
			if (count == n){
